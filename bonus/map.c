...
void	ft_put_img(int l, int k, t_program *prog)
{
	...
	{
		mlx_put_image_to_window(prog->mlx_p, prog->window,
			prog->img_cat[0], l * IMG_WIDTH, k * IMG_HEIGHT);
		prog->p_y = k;
		prog->p_x = l;
	}
	...
	else if (prog->map[k][l] == 'N')
		mlx_put_image_to_window(prog->mlx_p, prog->window,
			prog->img_8, l * IMG_WIDTH, k * IMG_HEIGHT);
}
...
void	ft_draw_map(t_program *prog, t_dim *dim)
{
	...
	mlx_string_put(prog->mlx_p, prog->window, 10, prog->score,
		0x00FF0000, "Movements:");
}
